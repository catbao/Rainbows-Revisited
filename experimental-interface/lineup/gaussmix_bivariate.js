...
GaussMixBivariate.prototype.sampleModel = function(iterations, _field)
{
    ...
    // console.log("_field:", _field.view);
    _field.normalize();
    _field.updated();

    var sum=0, max=0;
    for (var i = 0; i < _field.view.length; i++) {  
        for (var j = 0; j < _field.view[i].length; j++) {  
            if (_field.view[i][j] > max) {  
                max = _field.view[i][j];  
            }  
            sum += _field.view[i][j];
        }  
    }  
    // console.log("Sum of all values in _field:", sum);  
    // console.log("Maximum value in _field:", max);
}
